...
def new_logic():
    ...
    #TODO completar la creación del mapa HECHO
    catalog['books_by_year_author'] = sp.new_map(1000,0.7) 
...
def add_book_author_and_year(catalog, author_name, book):
    ...
    if author_value:
        ...
        if pub_year_value:
            ...
            pub_year_map = sp.new_map(1000,0.7)
            sp.put(pub_year_map,pub_year,book)
...
def get_book_info_by_book_id(catalog, good_reads_book_id):
    ...
    return sp.get(catalog['books_by_id'],good_reads_book_id)
...
def get_books_by_author(catalog, author_name):
    ...
    """
    #TODO Completar función de consulta HECHO
    return sp.get(catalog['books_by_authors'],author_name)
...
def get_books_by_tag(catalog, tag_name):
    ...
    """
    #TODO Completar función de consulta HECHO
    return sp.get(catalog['tags'],tag_name)
...
def get_books_by_author_pub_year(catalog, author_name, pub_year):
    ...
    author_map = sp.get(catalog['books_by_year_author'], author_name)
    if author_map:
        year_map = sp.get(author_map, pub_year)
        if year_map:
            resultado = year_map
    else:
        resultado = None
